...
function HUDLootScreen:begin_choose_card(peer_id, card_id)
  ...
  for i, pc in ipairs(cards) do
    ...
    local joker = true
    local texture, rect, coords = tweak_data.hud_icons:get_icon_data("downcard_overkill_deck")
...
function HUDLootScreen:show_item(peer_id)
  ...
  if panel:child("item") then
    ...
    local item_pc = lootdrop_data[6]
    local drop = lootdrop_data[9]
    local item_text = managers.localization:text(drop.name .. "_roguelike_lootdrop")
