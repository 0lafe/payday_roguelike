...
function StoryMissionsGui:_update_info(mission)
  ...
  if mission.id == "sm_2" and not mission.completed and managers.roguelike.save_data.highest_tier > 0 then
    placer:add_row(canvas:fine_text({
    ...
    placer:add_row(canvas:fine_text({
      wrap = true,
      word_wrap = true,
      text = managers.localization:text("menu_roguelike_tierskip_body"),
      font = small_font,
      font_size = small_font_size,
      color = text_col
    }), nil, 0)
